...
describe('jest-junit tests', () => {
  ...
  it('handles Cypress test failures', async () => {
    const fixturePath = path.join(__dirname, 'fixtures', 'cypress-junit.xml')
    const outputPath = path.join(__dirname, '__outputs__', 'cypress-junit.md')
    const filePath = normalizeFilePath(path.relative(__dirname, fixturePath))
    const fileContent = fs.readFileSync(fixturePath, {encoding: 'utf8'})
